...
for (const id of ids) {
  ...
  // radius and targetY are what the POD needs to aim its live plinth at the
  // same camera this picture was taken with, before it has a model of its own
  // to measure. Without them the plinth sits at some default distance and the
  // baked shadow lands nowhere near it.
  bots[id] = {
    px: meta.px,
    yaw: +meta.yaw.toFixed(4),
    pitch: +meta.pitch.toFixed(4),
    radius: +meta.radius.toFixed(4),
    targetY: +meta.targetY.toFixed(4),
  };
  const kb = (fs.statSync(file).size / 1024).toFixed(0);
  console.log(`  ${id}: ${meta.px}x${meta.px}  ${kb}KB  ${drawnPct.toFixed(0)}% bot  ${clearPct.toFixed(0)}% clear`);
  // Write the index after EVERY bot. A full roster bake is ten minutes of
  // SwiftShader, and an index written only at the end means any interruption
  // throws away every bot that had already succeeded — the PNGs survive on disk
  // but the runtime cannot see a poster that is not in the index.
  writeIndex();
  await page.close();
}
